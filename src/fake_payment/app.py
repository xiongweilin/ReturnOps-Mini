...
def _store(key: str, request: RefundRequest) -> dict:
    with _lock, _db() as conn:
        existing = conn.execute("SELECT * FROM refunds WHERE idempotency_key = ?", (key,)).fetchone()
        if existing is not None:
            conn.execute("UPDATE refunds SET post_count = post_count + 1 WHERE idempotency_key = ?", (key,))
            return dict(existing)
        refund_id = f"rf_{uuid.uuid4().hex[:18]}"
        conn.execute(
            "INSERT INTO refunds (idempotency_key, refund_id, amount_minor, currency, return_case_id, status) VALUES (?, ?, ?, ?, ?, 'succeeded')",
            (key, refund_id, request.amount_minor, request.currency, request.return_case_id),
        )
        return {"idempotency_key": key, "refund_id": refund_id, "amount_minor": request.amount_minor, "currency": request.currency, "return_case_id": request.return_case_id, "status": "succeeded", "post_count": 1}
...
def create_refund(
    ...
    if simulate == "timeout_after_processing":
        ...
        background.add_task(_send_webhook, row)
    return {"refund_id": row["refund_id"], "status": row["status"], "amount_minor": row["amount_minor"], "currency": row["currency"], "idempotency_key": row["idempotency_key"]}
...
def refund_by_key(key: str = Query(min_length=1)) -> dict:
    ...
    return {"refund_id": row["refund_id"], "status": row["status"], "amount_minor": row["amount_minor"], "currency": row["currency"], "idempotency_key": row["idempotency_key"], "post_count": row["post_count"]}
